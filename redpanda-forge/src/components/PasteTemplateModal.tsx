      ...
      <div className="w-[600px] bg-app-sidebar border border-app-border rounded-xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-app-border">
          <h2 className="text-[14px] font-bold text-white flex items-center gap-2">
            <FileJson size={16} className="text-app-accent" /> Import Template JSON
          </h2>
        ...
        {conflictTemplate ? (
          <>
            <div className="p-6">
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-5 flex items-start gap-4 mb-4">
                <AlertTriangle className="text-red-400 mt-0.5 shrink-0" size={24} />
                <div>
                  <h3 className="text-white font-bold text-[14px] mb-1">Template ID Conflict</h3>
                  <p className="text-app-muted text-[13px] leading-relaxed">
                    A template with ID <span className="text-red-300 font-mono">"{conflictTemplate.new.id}"</span> already exists.
                  </p>
                  
                  <div className="mt-4 flex flex-col gap-2 bg-black/30 p-3 rounded border border-white/5">
                    <div className="flex justify-between text-[12px]">
                      <span className="text-app-muted">Current Version:</span>
                      <span className="text-white font-mono">{conflictTemplate.old.version || '1.0.0'}</span>
                    </div>
                    <div className="flex justify-between text-[12px]">
                  ...
                  <p className="text-app-muted text-[13px] mt-4">
            ...
            {/* Footer */}
            <div className="px-6 py-4 border-t border-app-border bg-app-card flex justify-end gap-3">
              <button 
                onClick={() => setConflictTemplate(null)} 
                className="px-4 py-2 text-[12px] font-bold text-app-muted hover:text-white transition-colors"
              >
                GO BACK
              </button>
              <button 
                onClick={() => finalizeSave(conflictTemplate.new, true)} 
                className="px-6 py-2 bg-red-600 text-white rounded-md text-[12px] font-bold hover:bg-red-500 transition-all flex items-center gap-2"
              >
          ...
          <>
            <div className="p-6 space-y-5">
              <div className="flex justify-between items-end">
                <label className="text-[11px] text-app-muted uppercase tracking-[1px] font-bold block">
                  Paste JSON
                </label>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-app-muted italic">or</span>
                  <input 
                    type="file" 
                    accept=".json"
                    ref={fileInputRef}
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white rounded border border-app-border transition-colors text-[11px] font-[600]"
                  >
                    <Upload size={14} className="text-app-accent" />
                    Upload .json File
                  </button>
                </div>
              </div>
              ...
              <div>
                <textarea 
                  ...
                  className="w-full h-[320px] bg-[#0A0A0C] border border-app-border rounded-md px-4 py-4 text-[#a0c5e8] font-mono text-[12px] outline-none focus:border-app-accent resize-none placeholder:text-app-muted/50 overflow-auto shadow-inner leading-relaxed"
                  value={jsonText}
                  onChange={(e) => {
                  ...
                  spellCheck={false}
                />
                {error && <p className="text-red-500 text-[12px] mt-2 flex items-center gap-1.5"><AlertTriangle size={14} /> {error}</p>}
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-app-border bg-app-card flex justify-end gap-3">
              ...
              <button 
                ...
                className="px-6 py-2 bg-app-accent text-white rounded-md text-[12px] font-bold hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                IMPORT
              </button>
            </div>
